...
#### Automated Verification:
- [x] Python compute + tests: `.venv/bin/pytest tests/actuarial/test_lapse.py tests/actuarial/test_kafka.py --no-cov` — 26/26 green. New `test_lapse.py` covers hand-calc (0.05/0.10, 0.12/0.15, 0.28/0.25), missing-expected → null A/E, full-retention-expected → null A/E, zero cohort size skip + warning, negative still_active clamp + warning, still_active > cohort_size clamp + warning, per-line split, dict→typed round-trip, empty input, JSON serialisation, and the "lapse + retention = 1.0" by-construction invariant against the persistency compute. `test_kafka.py::test_lapse_study_dispatches_via_cohort_branch` pins the runner routes `LAPSE_STUDY` through the new cohort branch.
- [x] Full ai-service suite unchanged: `.venv/bin/pytest --no-cov` — 99/99 green.
- [x] Java compile + shaping unit tests: `:finance-service:compileJava :finance-service:compileTestJava` green; `LapseCohortShapingServiceTest` 3/3 green (feed + basis rows → cohort payload shape with `still_active` key, empty feed → warning, inverted period rejected).
- [x] Angular unit tests: `ng test --include='src/app/core/services/actuarial-reports.service.spec.ts'` — 7/7 green including new `POST /reports/actuarial/lapse-study` assertion.
- [x] Angular build: `npx ng build --configuration=development` — bundle generation complete; only pre-existing warnings (identical set to Phase 10/11).
- [ ] Java IT: `LapseReportIT` deferred. Same rationale as Phase 11 §Deviations — `ActuarialJobFullPathIT` already exercises the publisher/consumer/repository plumbing that `submitLapseStudy` reuses; the only Phase-12-specific piece (`LapseCohortShapingService.assemble()`) is unit-tested. Adding an IT would need a fresh cohort feed fixture on top of the ActuarialJobFullPathIT baseline, which is a heavier lift than the Phase-14-integration follow-up already promised at §What we're not doing.
- [ ] Playwright: `actuarial-lapse-study.spec.ts` — deferred to the CI runner alongside the other e2e specs (mirrors the Phase 3 / 6 / 10 / 11 deferrals in this document).
...
#### Automated Verification:
- [x] Python: A/E on canonical basis-hand-calc matches to 6 dp. `test_mortality.py::test_ae_ratio_hand_calc_against_a1949_52` pins the A1949-52 interpolation (age-32 male → 2.34 per 1000) + expected-deaths + A/E arithmetic against the shipped YAML, and `test_ae_ratio_near_one_when_deaths_match_basis` shows A/E → 1.0 when observed deaths exactly match the basis rate. Full basis coverage sanity check (`test_every_mortality_basis_loads_and_computes`) confirms all 4 mortality YAMLs load + compute. 25/25 mortality-compute tests green in `.venv/bin/pytest tests/actuarial/test_mortality.py --no-cov`.
- [x] Kafka runner routes MORTALITY_STUDY through the new exposure branch — `test_kafka.py::test_mortality_study_dispatches_via_exposure_branch` pins the runner produces a `MORTALITY_STUDY` completed envelope from an exposure payload; `test_mortality_missing_exposure_publishes_failed` pins the missing-exposure failure path. The pre-existing `unknown_report_key_publishes_failed` was pointed at `MORBIDITY_STUDY` (still not wired until Phase 14). 17/17 kafka tests green.
- [x] Full ai-service suite unchanged: `.venv/bin/pytest --no-cov` — 126/126 green (101 sibling + 25 new mortality). The 70% aggregate coverage gate is pre-existing (same rationale as Phases 7-12).
- [x] Java compile + shaping unit tests: `:finance-service:compileJava :finance-service:compileTestJava :user-service:compileJava` all green. `MortalityExposureShapingServiceTest` 6/6 green — feed + basis rows → exposure payload shape, missing basis → fallback + warning, override precedence, empty feed → warning, null line → HEALTH default, inverted period rejected.
- [x] Angular unit tests: `ng test --include='src/app/core/services/actuarial-reports.service.spec.ts' --watch=false` — 8/8 green including the new `POST /reports/actuarial/mortality-study` assertion.
- [x] Angular build: `npx ng build --configuration=development` — new mortality-study component + template + route land cleanly. Only pre-existing warnings surface (identical set to Phases 10-12).
- [ ] Java IT: `MortalityReportIT` deferred. `ActuarialJobFullPathIT` covers the IBNR path through the same publisher/consumer/repository plumbing that MORTALITY_STUDY reuses; the only Phase-13-specific piece (`MortalityExposureShapingService.assemble()` + user-service exposure feed SQL) is unit-tested. Adding an IT would need `members` fixture rows with death_date + gender + date_of_birth + enrollment_date + optional life_policies rows for the LIFE line branch — a heavier lift than the Phase-14-integration follow-up already promised at §What we're not doing.
- [ ] Playwright: `actuarial-mortality-study.spec.ts` — deferred to the CI runner alongside the other e2e specs (mirrors the Phase 3 / 6 / 10 / 11 / 12 deferrals in this document).
...
#### Automated Verification:
- [x] Python compute + tests: `.venv/bin/pytest tests/actuarial/test_morbidity.py tests/actuarial/test_kafka.py --no-cov` — 44/44 green. New `test_morbidity.py` (25 tests) covers hand-calc (CIDA age-32 male interpolated to 2.14/1000), missing-expected → null A/E, negative-incidents clamp + warning, zero-exposure skip + warning, unknown basis / mortality basis rejected as morbidity, unknown sex / unparseable band skip + warning, per-line split, duplicate line ignored + warned, dict→typed round-trip, empty input, JSON serialisation, and the A/E=1.0 sanity check when observed incidents exactly match expected. `test_kafka.py::test_morbidity_study_dispatches_via_exposure_branch` + `test_morbidity_missing_exposure_publishes_failed` pin the runner branch; the pre-existing `test_unknown_report_key_publishes_failed` was pointed at a fabricated `NOT_A_REAL_KEY` since MORBIDITY_STUDY is no longer unknown.
- [x] Full ai-service suite unchanged: `.venv/bin/pytest --no-cov` — 153/153 green (126 sibling + 25 new morbidity + 2 new morbidity kafka tests).
- [x] Java compile + shaping unit tests: `:finance-service:compileJava :finance-service:compileTestJava :user-service:compileJava :user-service:compileTestJava` all green. `MorbidityExposureShapingServiceTest` 6/6 green — feed + basis rows → exposure payload shape with `incidents` key, missing basis → CIDA fallback + warning, override precedence, empty feed → warning, null line → HEALTH default, inverted period rejected.
- [x] Angular unit tests: `ng test --include='src/app/core/services/actuarial-reports.service.spec.ts' --watch=false` — 9/9 green including the new `POST /reports/actuarial/morbidity-study` assertion.
- [x] Angular build: `npx ng build --configuration=development` — new morbidity-study component + template + route land cleanly. Only pre-existing warnings surface (identical set to Phases 10-13).
- [ ] Java IT: `MorbidityReportIT` deferred. Same rationale as Phase 11 / 12 / 13 §Deviations — `ActuarialJobFullPathIT` already exercises the publisher/consumer/repository plumbing that `submitMorbidityStudy` reuses; the only Phase-14-specific pieces (`MorbidityExposureShapingService.assemble()` + user-service morbidity feed SQL) are unit-tested. Adding an IT would need `members` fixture rows with date_of_birth + gender + enrollment_date + `member_status_history` entries with the morbidity reason_code vocab — a heavier lift than the Phase-14-integration follow-up already promised at §What we're not doing.
- [ ] Playwright: `actuarial-morbidity-study.spec.ts` — deferred to the CI runner alongside the other e2e specs (mirrors the Phase 3 / 6 / 10-13 deferrals in this document).
...
#### Automated Verification:
- [x] Java compile + tests: `./gradlew :rules-engine:compileJava :rules-engine:compileTestJava :rules-engine:test` green. New `ActuarialTemplatesTest` (5 methods) compiles each template to DRL, loads into `TenantRuleEngine`, fires via `evaluateInGroup("ACTUARIAL")` and asserts the ldfMethod that lands on `TriangleFact` per line. New `SelectLdfEmitterTest` (7 methods) pins the DRL-string encoding branches. `DrlCompilerTest` gains an ACTUARIAL case asserting the agenda-group + auto-bound `$triangle` + `$triangle.selectLdf(...)` shape.
- [x] `TenantActuarialRulesIT` — hand-authored rule + `TenantRuleEngine.loadRules` + `evaluateInGroup("ACTUARIAL")` asserts ldfMethod = "5yr"; a plain `evaluate()` (unfocused stage-7 sweep) asserts the rule stays dormant; a non-matching line asserts default volume.
...
#### Automated Verification:
- [x] Java compile + unit tests: `./gradlew :finance-service:compileJava :finance-service:compileTestJava :finance-service:test --tests 'com.medfund.finance.actuarial.*'` all green. New `ActuarialRulesEvaluatorTest` (4 methods) covers no-rule → volume default, rule-fires → mutated method + audit-line rule name, engine-failure → volume fallback, null insuranceLine → "ALL" on the built fact. New `ActuarialXlsxServiceTest` (2 methods) covers the "Rule applied: …" header on Triangle + Summary sheets and the "—" sentinel when no rule fired.
- [x] Angular compile + unit tests: `npx ng build --configuration=development` succeeds; `ng test --include='src/app/core/services/rules.service.spec.ts'` 5/5 green (including a new "Phase 14 ACTUARIAL category" assertion); `ng test --include='src/app/pages/tenant-admin/rules/**/*.spec.ts'` 4/4 green.
- [ ] Java IT: `ActuarialRulesIntegrationIT` — deferred to the Phase-14-integration follow-up alongside the six golden-path e2e specs. `ActuarialJobFullPathIT` continues to cover the publisher / consumer / repository plumbing that Phase 16 reuses; the only Phase-16-specific piece (`ActuarialRulesEvaluator` insertion into the submit path) is unit-tested. Adding an IT would need a per-tenant rule fixture on top of the ActuarialJobFullPathIT baseline plus a TenantRuleLoader Spring bean — cost/coverage matches the sibling Phase 11 / 12 / 13 IT deferrals.
- [ ] Playwright: rules authoring flow spec — deferred to the CI runner alongside the other e2e specs (mirrors Phases 3 / 6 / 10 / 11 / 12 / 13 / 14 deferrals).
- [x] Per-tranche migration folder — **not applicable**: no schema changes in Phase 16. `params_json` gains an optional `ldfRuleApplied` key but the column is already JSONB; additive.
...
- **2026-08-27 — Phase 12 reuses the Phase 11 persistency-cohort feed rather than adding a lapse-specific endpoint.** The plan's Phase 12 §Changes Required section left the feed source implicit ("Reads Phase-13 `policy_status_history` for annual lines; `member_status_history` for HEALTH"). The existing `/api/v1/reports/policy-lifecycle/persistency-cohort-feed` already returns exactly that — one row per `(cohortMonth, insuranceLine, checkpointMonths, cohortSize, stillActive)` — from which lapse = `cohort_size - still_active` is a straight subtraction. `LapseCohortShapingService` therefore calls `UserServiceClient.persistencyCohortFeed(...)` verbatim and relabels the payload key `still_active` (Python's lapse compute derives `lapsed_count` internally). This keeps the "lapse + retention = 1.0" and "lapse rate reconciles against POLICY_MOVEMENT" success criteria true by construction — both reports flow through the same SQL. The Phase 11 §Deviation that consolidated the two feeds into one endpoint predicted this too.
- **2026-08-27 — Phase 12 XLSX export follows Phase 11's approach: no cohort-specific rendering.** The existing `ActuarialXlsxService` writes a Triangle sheet (falls through to the "Triangle input not preserved" fallback for cohort jobs), a Development-factors sheet (empty for cohort jobs), and a Summary sheet (0s for cohort jobs). Phase 11 accepted this shape rather than adding a cohort-shaped renderer; Phase 12 preserves parity. A cohort-shaped XLSX renderer is a natural follow-up alongside the Phase 13/14 mortality/morbidity studies, which will hit the same limitation.
...
- **2026-08-27 — Phase 13 mortality exposure feed is a single user-service endpoint, not a "members" REST addition.** The plan sketched `HTTP GET to new user-service /api/v1/members/exposure`. The exposure query joins `members` with an optional per-line population filter (health-cohort matview / life_policies / funeral_policies / disability_policies / travel_policies) so it lives next to the other policy-lifecycle feeds at `/api/v1/reports/policy-lifecycle/mortality-exposure-feed`. Same rationale as the Phase 11 §Deviation — one place, one repository class, one permission surface (`FINANCE_VIEW_SUBLEDGER`), no `@RequiresReport` gate so MORTALITY_STUDY still runs if a sibling policy-lifecycle report is toggled off.
- **2026-08-27 — Phase 13 basis-name fallback lives inside the shaping service, not the compute.** The plan's Phase-13 note said "reads YAML basis, applies multiplier from `tenant_mortality_basis`" without specifying who picks the basis when the tenant has none configured. Shipped code has `MortalityExposureShapingService` fall back to `A1949_52` with multiplier 1.0 and emit a warning; the Python compute keeps its "skip cohorts with unknown basis" semantics. Consequence: an early-dev tenant with no `tenant_mortality_basis` row still gets a numerical A/E envelope (not a hard failure), matching the Phase-13 manual-verification acceptance ("may be empty in early dev — assert empty envelope with warnings rather than error").
- **2026-08-27 — Phase 14 morbidity incidence signal derives from `member_status_history.reason_code`, not claims-service claim onsets.** The plan's Phase-14 note said "Reads claim onset dates + Member exposure + tenant_morbidity_basis." Claims live in claims-service, not user-service; a cross-service feed is out of scope for the final non-rules phase. Shipped code counts DISTINCT members whose `member_status_history` has a `reason_code IN ('illness_onset','disability_onset','hospitalization')` transition in the window — the same reason-code channel the Phase-13 death recording flows through. Absent codes yield `incidents=0` per member, so early-dev tenants get an empty envelope with warnings (matching the "may be empty in early dev — assert empty envelope with warnings rather than error" acceptance from Phase 13). A follow-up phase will thread the true claim-onset signal from claims-service via a new feed endpoint.
- **2026-08-27 — Phase 14 morbidity feed lives at `/api/v1/reports/policy-lifecycle/morbidity-incidence-feed`, matching the Phase-13 mortality feed convention.** The plan sketched a `MorbidityShapingService` without specifying the feed location. Following the Phase 11 / 13 §Deviations that consolidated cross-service feeds into `/reports/policy-lifecycle/*` (not `/members/*` or `/policies/*`), the morbidity feed lives beside the mortality one — same repository class, same `FINANCE_VIEW_SUBLEDGER` permission, same no-`@RequiresReport` carve-out so MORBIDITY_STUDY runs even when a sibling policy-lifecycle report is toggled off.
- **2026-08-27 — Phase 16 introduces `ActuarialRulesEvaluator` as the rules-engine bridge, not an inline call inside `TriangleShapingService`.** The plan sketched a `rulesEngineClient.evaluate(tenantId, RuleCategory.ACTUARIAL, fact)` call inside `TriangleShapingService.shape(...)` and read the ldfMethod back off the fact. Shipped code hoists that plumbing into a dedicated `com.medfund.finance.actuarial.service.ActuarialRulesEvaluator` component with a `Selection(method, ruleName)` record output. Rationale: the rules-engine's returned `List<RuleResult>` is empty for non-ClaimFact facts (matches the `PremiumFact` / `ContributionFact` conventions used by `CommissionCalcService`); the caller has to read mutations off the fact instance directly. Wrapping that idiom + the `TenantRuleLoader.ensureLoaded` + agenda-focus + rule-name capture inside a single service keeps `TriangleShapingService` unchanged (pure shape logic) and keeps the plumbing testable in isolation — mirrors the same split `CommissionCalcService` uses for the COMMISSION agenda group. The user-facing behaviour matches the plan sketch: `params_json.ldfMethod` reflects the selected method, `params_json.ldfRuleApplied` reflects the rule audit line, and the XLSX header shows both.
- **2026-08-27 — Phase 16 wires the rules bridge into `ActuarialJobService.submit(...)`, not `TriangleShapingService.shape(...)`.** The plan sketched the rules-engine call inside the shaper because the fact needed line + grain + period; the shipped location is `ActuarialJobService` because `submitWithCurrency` already has the resolved currency and the request DTO in hand, so building the `ActuarialRuleRequest` there costs less passing of primitives. The shaper stays a pure claims-in → matrix-out function. Behaviour is identical: the selected method + rule name land in `params_json` before the row is inserted, so the params-hash + Kafka event + XLSX export + polling UI all see the final chosen method.
- **2026-08-27 — Phase 16 caller-side `request.ldfMethod` override wins over tenant rules.** The plan's Phase 16 §1 sketch was silent on precedence when a caller sends `ldfMethod: "simple"` in the submit body AND the tenant has a matching ACTUARIAL rule. Shipped code short-circuits the rule evaluation when the caller pins a method — the "user override" path stays explicit for ad-hoc actuarial exploration. Rule-driven selection covers the "default per line, per period" path where the tenant admin doesn't want to touch every submit; explicit override covers the "run one comparison with a specific method" path.
