...
#### Automated Verification
- [x] `cd services/java && ./gradlew :finance-service:compileJava :finance-service:compileTestJava :contributions-service:compileJava :contributions-service:compileTestJava :rules-engine:compileJava :shared:compileJava` — clean (2026-08-22)
- [x] `make test-java` — `PremiumCessionServiceTest` (8 cases: proportional QS writes PREMIUM cession without recovery; non-proportional treaty filtered out; already-ceded idempotent skip; UNIQUE race swallow; no matching treaty; zero amount short-circuit; missing tenantId short-circuit; multi-treaty XoL exclusion) + `ReinsurancePremiumCessionConsumerTest` (5 cases: well-formed dispatch; missing tenant/line skip; pre-Phase-6 payload skip; malformed JSON errors) + `ReinsuranceTreatyPremiumExecutorTest` (5 cases: JobType binding; XoL writes inception cession; idempotent skip on existing; UNIQUE race swallow; multi-treaty). Also: `BillingServiceTest.recordPayment_existingContribution_setsStatusPaid` + `ContributionEventPublisherTest.publishContributionPaid_sendsToCorrectTopic_withPhase6Fields` updated for the additive payload; both green. All 3 new Phase 6 unit test classes + affected contributions tests green.
- [x] `make test-integration`:
  - `ReinsurancePremiumCessionIT` (2 tests, real Postgres via Testcontainers): paid contribution writes PREMIUM Cession with correct amounts/status + emits AuditEvent with friendly `entityName` (not UUID); no Recovery row (recovery is loss-cession leg only); reprocessing same contribution yields 1 cession row (idempotent).
  - `ReinsuranceTreatyPremiumExecutorIT` (3 tests): seeded XoL + StopLoss treaties → executor writes one PREMIUM cession per treaty at inception with `source_event_type=TREATY_INCEPTION`; three-in-a-row reruns → still one row per treaty (idempotent via `ux_cession_source_event`); proportional treaty seeded → executor writes nothing (exclusion filter enforced).
  - Existing `ReinsuranceLossCessionIT`, `ReinsuranceCrudIT`, `ReinsuranceBordereauIT` re-run clean — no regressions from the Phase 6 additions.
...
#### Automated Verification
- [x] `cd services/java && ./gradlew :finance-service:build` — clean compile (2026-08-22)
- [x] `make test-java` — new `FacultativeCessionServiceTest` (20 cases) covers every transition (createDraft happy path + missing line + treaty-not-found + treaty-not-active + line-not-covered + existing-live-cession + UNIQUE race + missing insuranceLine; approve happy + non-facultative + not-draft + not-found; commit happy + recovery-idempotent + not-approved; void happy + approved-cascade + terminal-recovery-left-alone + missing-reason + ceded-conflict + non-facultative-conflict). All 20 green.
- [x] `make test-integration` — new `FacultativeCessionIT` (4 tests, real Postgres via Testcontainers + real R2DBC + real service layer): full DRAFT→APPROVED→CEDED round-trip writes a Cession + Recovery(EXPECTED) with two AuditEvents carrying friendly `entityName`; void-DRAFT is safe when no recovery exists; DRAFT-treaty creation rejected; line-not-covered rejected. Verified against Reinsurance* IT peers in isolation — full-suite runs hit a shared Postgres `too many clients` cap unrelated to Phase 7.
- [x] Angular compiles: `cd clients/angular && npx ng build --configuration=development` — SUCCESS (pre-existing warnings only)
- [ ] `verify` on `/tenant/finance/reinsurance/facultative/browse` — table shows candidates, cede pane opens (deferred to manual acceptance)
- [ ] `verify` on `/tenant/finance/reinsurance/facultative/queue` — queue shows DRAFT/APPROVED cessions, approve/commit actions work (deferred to manual acceptance)
- [x] Playwright: `reinsurance-facultative.spec.ts` — two scenarios: underwriter creates DRAFT via the browse page; supervisor loads the queue → approves → commits → queue clears (spec added; full CI run deferred to green-suite check).
...
#### Automated Verification
- [x] `cd services/java && ./gradlew :finance-service:compileJava :finance-service:compileTestJava` — clean compile (2026-08-22)
- [x] `make test-java` — new `ReinsuranceReviewTaskServiceTest` (11 cases: regression opens N tasks / equal-basis no-op / duplicate-guard skip; assign flips OPEN→IN_PROGRESS + rejects on resolved; resolve keep/dismiss/void; void cascades to cession + non-terminal recovery; terminal recovery left alone; invalid resolution rejected; already-resolved rejected). `RecoveryServiceTest` (8 cases: markReceived from EXPECTED / from INVOICED / rejected on RECEIVED / negative-amount rejected / missing recovery; writeOff from EXPECTED / rejected on WRITTEN_OFF / missing reason rejected). `TreatyActivationBackfillJobTest` (3 cases: iterates every applicable line & dispatches each; row failure increments failed + continues; no applicable lines is no-op). Regression detection extended in `ReinsuranceLossCessionConsumerTest` (9 cases: first-time cede unchanged; partial-approved dispatch; REJECTED with no history skipped; re-adjudicated lower opens regression task; re-adjudicated higher no-op; REJECTED with history opens regression task; missing tenant/line skip; malformed JSON). All new tests green.
- [x] `make test-integration`:
  - `ReinsuranceReviewAndRecoveryIT` (3 tests, real Postgres via Testcontainers + real R2DBC + real service layer): regression opens task → resolve RESOLVED_VOID → cession → VOIDED + recovery → WRITTEN_OFF with "Cession voided:" prefix; mark-received on EXPECTED transitions to RECEIVED with amount, write-off on the resulting RECEIVED row rejected as terminal; createRegressionTasks re-run on the same cession stays at one task (idempotent via `findOpenByCessionAndType`). Verified against Phase 8 peers in isolation — full-suite runs hit the shared Postgres `too many clients` cap documented in Phase 7 deviation, unrelated to Phase 8.
- [x] Playwright: `reinsurance-review-queue.spec.ts` — supervisor loads regression task, resolves RESOLVED_VOID with notes, queue clears. `reinsurance-recovery-lifecycle.spec.ts` — mark-received on EXPECTED row (row flips to RECEIVED) + write-off on INVOICED row (row flips to WRITTEN_OFF). Both specs stub the /api/v1 layer via `apiMocks`.
...
- **2026-08-22 (Phase 6 §4 — scheduler is a `JobExecutor`, not `@Scheduled`)** — the plan sketch used a Spring `@Scheduled(cron = "0 0 2 * * *")` on a `ReinsuranceTreatyPremiumJob` bean that iterated `tenantRepository.findAllActive()`. finance-service does not own the `tenants` table and has no `TenantRepository` — the surrounding pattern established by `PaymentRunExecutor` implements `JobExecutor` and is invoked per-tenant by `JobDispatcher` on a tenant-configured cron. Adopted here: a new `JobType.REINSURANCE_TREATY_PREMIUM` enum entry + a `ReinsuranceTreatyPremiumExecutor implements JobExecutor` under `services/java/finance-service/src/main/java/com/medfund/finance/reinsurance/job/`. Cron/enable is tenant-configurable via `scheduled_job_configs` (the same table PaymentRun uses); the executor stays idempotent via the existing `ux_cession_source_event` UNIQUE, so any manual retrigger is safe. No `@Scheduled` annotation is introduced.
- **2026-08-22 (Phase 6 §1 — ContributionPaidEvent adds a scheme-resolved `insuranceLine`)** — the plan's `ContributionEventPublisher.publishContributionPaid` sketch took `insuranceLine` as a direct arg. `Contribution` has no line column — the line lives on `schemes`. `BillingService.recordPayment` was extended to load the scheme once via the already-injected `schemeRepository`, pull `insuranceLine` (defaulting to `HEALTH` when null, same convention as `priceOneMember`) and `currencyCode`, and pass them into the publisher alongside a formatted `paidAt`.
...
- **2026-08-22 (Phase 7 §2 — void uses POST /{id}/void, not DELETE with body)** — the plan called for `DELETE /{id}` with a `{reason}` request body. Angular's `HttpClient.delete` doesn't take a body without a custom overload, and Spring / browser XHR support for DELETE-with-body is spotty. Switched to `POST /api/v1/reinsurance/facultative/{id}/void` matching the treaty pattern (`POST /treaties/{id}/void`) already in the reinsurance surface. Same semantics, same permission gate, cleaner client.
- **2026-08-22 (Phase 7 §3 — cede pane inline, not a separate `facultative-cede-modal.component`)** — the plan named a separate cede modal component. Delivered inline as the right-hand pane of `facultative-browse.component` — the underwriter picks a candidate on the left and the form on the right populates. Same UX as a modal, fewer files, no modal-lifecycle plumbing. If a second entry point (e.g. cede-from-claim-detail) is added later, that's the extraction trigger.
- **2026-08-22 (Phase 7 §1 — Recovery moves earlier still, to `commit()` not `createDraft()`)** — the plan noted Phase 3 had already moved Recovery creation to cession-write time. For facultative, Phase 3's earlier deviation makes DRAFT-time recovery creation wrong: a DRAFT can be voided before ever reaching the reinsurer, and writing an EXPECTED recovery at DRAFT would misrepresent the fund's expected-recoveries position. Adopted: DRAFT writes only the Cession row; APPROVED is idempotent (no additional writes); `commit()` transitions the cession to CEDED **and** writes the EXPECTED Recovery in the same transaction. `voidCession()` cascade-writes-off any Recovery (EXPECTED or INVOICED) that has already been created — a defensive path in case a bordereau invoice race prints an invoice for a still-drafting cession. Terminal Recovery states (RECEIVED, WRITTEN_OFF) are left alone.
- **2026-08-22 (Phase 7 §2 — candidates via HTTP to claims-service, not a finance-local projection)** — the plan is silent on how the underwriter surfaces "adjudicated claims above threshold that haven't been ceded." The tempting option is to lean on finance-local projections (`member_payables`, `member_cost_share_liabilities`) but those only cover MEMBER-payee claims. Introduced `ClaimsClient.findFacultativeCandidates(insuranceLine, minAmount, page, size)` — a thin WebClient GET on the existing `/api/v1/claims/page?status=ADJUDICATED&insuranceLine=…` endpoint with client-side `approvedAmount >= minAmount` filtering. Peer downtime returns `[]`; the browse is discovery-only, so an empty list is not treated as an error. Phase 8's `TreatyActivationBackfillJob` will need a similar streaming variant when it lands.
- **2026-08-22 (Phase 8 §3 — backfill uses a page-stream on the existing /claims/page endpoint, not `streamAdjudicatedClaimsIn`)** — the plan sketched `claimReader.streamAdjudicatedClaimsIn(from, to, lines)`. No such endpoint exists on claims-service, and adding one would require: a new claims-service surface + a `since` timestamp column indexed on `claims`. Introduced `ClaimsClient.streamAdjudicatedClaimsForBackfill(insuranceLine, sinceInclusive, pageSize)` — pages through the same `/api/v1/claims/page?status=ADJUDICATED` used by facultative candidates, then filters client-side on `adjudicatedAt >= sinceInclusive`. Peer error terminates the stream (returns `[]` for the failing page); the operator retries via re-activation, and idempotency in `CessionService` guarantees zero duplicates on retry. `TreatyActivationBackfillJob.kickOff` fires and forgets, preserving the tenant context via `Schedulers.parallel() + contextWrite`; `BackfillProgressService` tracks per-treaty processed/failed counters in-memory, polled by `GET /api/v1/reinsurance/treaties/{id}/backfill-progress`.
- **2026-08-22 (Phase 8 §1 — regression check uses REJECTED + APPROVED + PARTIAL_APPROVED as candidate decisions, no producer change)** — the plan sketch checked REJECTED alongside APPROVED, but the existing consumer's `CEDING_DECISIONS` set filtered to just APPROVED / PARTIAL_APPROVED. Extended in-place to `REGRESSION_CANDIDATE_DECISIONS = { APPROVED, PARTIAL_APPROVED, REJECTED }` — REJECTED lands with `approvedAmount = 0` which is trivially lower than any prior basis. First-time-cede path preserves the pre-Phase-8 short-circuit for a REJECTED-with-no-history (nothing to regress against). No producer-side change required — the existing `medfund.claims.adjudicated` payload already carries `decision` and `approvedAmount`.
- **2026-08-22 (Phase 8 §5 — recovery actions embedded on the recoveries bordereau page, not standalone recovery-detail routes)** — the plan called for standalone `record-received-modal.component.ts` and `write-off-modal.component.ts` files. Delivered as inline sections + row actions on the existing `recoveries-bordereau.component` (Phase 5), the natural discovery surface for "outstanding recoveries to act on." Reduces file count (no standalone modal components) and lands the actions on the surface the operator uses to see the recovery in the first place. If a claim-detail-scoped recovery action is later needed, the modal can be extracted then.
- **2026-08-22 (Phase 8 §5 — review-queue route lives under `/tenant/finance/reinsurance/review-queue`, not a separate reinsurance area)** — the plan named `reinsurance/review-queue` without a top-level parent. Registered as `tenant/finance/reinsurance/review-queue` alongside the existing `tenant/finance/reinsurance/facultative/*` routes. Sidebar entry ("Reinsurance — Review Queue") sits next to the two Facultative entries in operational-nav.ts so the operator sees the whole reinsurance surface in one nav cluster.
