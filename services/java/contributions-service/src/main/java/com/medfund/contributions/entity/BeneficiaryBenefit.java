 ...
 * Populated on enrolment (deferred hook) or via V060's backfill;
 * {@code consumed_amount} + {@code consumed_count} accrue every time
 * a claim line against this benefit is accepted by an adjudicator.
 *
 * <p>{@code dependant_id} is NULL when the beneficiary IS the member —
 * the member's own benefit row.
 */
