...
@Repository
public class LifePolicyQueryRepository {
    ...
    public LifePolicyQueryRepository(DatabaseClient db) {
        this.db = db;
    }

    public Flux<LifePolicyRow> search(LifePolicyFilterParams f, int limit, int offset) {
        boolean hasQ = f.q() != null && !f.q().isBlank();
        String search = hasQ ? "%" + f.q().toLowerCase() + "%" : null;

        String sql = """
                SELECT p.id, p.scheme_id, p.insured_member_id,
                       p.policy_number, p.sum_assured, p.occupation_hazard_class,
                       p.term_months, p.status,
    ...
    public Mono<Long> count(LifePolicyFilterParams f) {
        boolean hasQ = f.q() != null && !f.q().isBlank();
        String search = hasQ ? "%" + f.q().toLowerCase() + "%" : null;

        String sql = """
                SELECT COUNT(*) AS total FROM life_policies p
                  LEFT JOIN schemes s ON s.id = p.scheme_id
                  LEFT JOIN members m ON m.id = p.insured_member_id
                """
                + whereClause(f, hasQ);
    ...
    private String whereClause(LifePolicyFilterParams f, boolean hasQ) {
    ...
    private LifePolicyRow toRow(io.r2dbc.spi.Readable row) {
        ...
        return new LifePolicyRow(
                ...
                row.get("sum_assured", BigDecimal.class),
