...
import com.medfund.finance.regulatory.naic.NaicScheduleFCalculator;
import com.medfund.finance.regulatory.naic.NaicScheduleFRawData;
import com.medfund.finance.regulatory.naic.NaicScheduleFReportShaper;
...
class NaicScheduleFJobServiceTest {
    ...
    void setUp() {
        ...
        service = new NaicScheduleFJobService(jobRepository, shapingService, xlsxService,
                submissionService, auditPublisher, mapper, Optional.of(configReader));
    }

    @Test
    void submit_rejectsClientCurrencyOverride_with422() {
        NaicScheduleFReportRequest override = new NaicScheduleFReportRequest(PERIOD_START, PERIOD_END,
                "ZAR", false, null);
    ...
    void submit_rejectsWith422_whenConfigReaderBeanAbsent() {
        // No configReader wired → Phase 13 pre-Phase-14 default: gate fails closed.
        ObjectMapper mapper = new ObjectMapper().registerModule(new JavaTimeModule());
        NaicScheduleFJobService svcNoReader = new NaicScheduleFJobService(
                jobRepository, shapingService, xlsxService, submissionService,
                auditPublisher, mapper, Optional.empty());

        NaicScheduleFReportRequest req = new NaicScheduleFReportRequest(PERIOD_START, PERIOD_END,
                null, false, null);
    ...
    void submit_dedupesInflightJob_returnsExistingJobId_whenConfigPresent() {
        ReportJob existing = jobRow(UUID.randomUUID(), NaicScheduleFJobService.STATUS_PROCESSING);
        ...
        StepVerifier.create(service.submit(req, TENANT, null, ACTOR.toString(), ACTOR_EMAIL))
                .assertNext(resp -> {
                    ...
                    assertThat(resp.status()).isEqualTo(NaicScheduleFJobService.STATUS_PROCESSING);
                })
                .verifyComplete();
    ...
    void submit_insertsNewJob_setsPrudentialStatutoryRetention_andReturnsJobId() {
        ...
        when(shapingService.shape(eq(ReportKey.NAIC_SCHEDULE_F),
        ...
        StepVerifier.create(service.submit(req, TENANT, null, ACTOR.toString(), ACTOR_EMAIL))
                .assertNext(resp -> {
                    ...
                    assertThat(resp.status()).isEqualTo(NaicScheduleFJobService.STATUS_REQUESTED);
                })
                .verifyComplete();
    }
    ...
    void computeChain_dryRun_flipsToCompleted_withSectionsInResultJson() {
        ReportJob row = jobRow(UUID.randomUUID(), NaicScheduleFJobService.STATUS_REQUESTED);
        ...
        StepVerifier.create(service.computeChain(row, req, TENANT, null, ACTOR.toString(), ACTOR_EMAIL))
                .assertNext(saved -> {
                    assertThat(saved.getStatus()).isEqualTo(NaicScheduleFJobService.STATUS_COMPLETED);
                    assertThat(saved.getCompletedAt()).isNotNull();
                    assertThat(saved.getResultJson()).isNotNull();
                    String rj = saved.getResultJson().asString();
                    assertThat(rj).contains("\"" + NaicScheduleFJobService.RESULT_SECTIONS + "\"");
                    assertThat(rj).doesNotContain("\"" + NaicScheduleFJobService.RESULT_SUBMISSION_ID + "\"");
                })
                .verifyComplete();
    ...
    void computeChain_submitFlagTrue_archivesViaSubmissionService_andRecordsSubmissionId() {
        ...
        when(xlsxService.render(eq(TENANT), any(RegulatoryReportData.class)))
                .thenReturn(Mono.just(new NaicScheduleFXlsxService.NaicScheduleFRenderResult(
        ...
        when(submissionService.submit(eq(TENANT), eq(ReportKey.NAIC_SCHEDULE_F.name()),
        ...
        StepVerifier.create(service.computeChain(row, req, TENANT, null, ACTOR.toString(), ACTOR_EMAIL))
                .assertNext(saved -> {
                    ...
                    assertThat(saved.getResultJson().asString())
                            .contains("\"" + NaicScheduleFJobService.RESULT_SUBMISSION_ID + "\"")
    ...
    void computeChain_shapingError_flipsToFailed_withErrorMessage() {
        ...
        StepVerifier.create(service.computeChain(row, req, TENANT, null, ACTOR.toString(), ACTOR_EMAIL))
                .assertNext(saved -> {
                    assertThat(saved.getStatus()).isEqualTo(NaicScheduleFJobService.STATUS_FAILED);
    ...
    void get_rejectsCrossTenantJob_with404() {
        ReportJob row = jobRow(UUID.randomUUID(), NaicScheduleFJobService.STATUS_COMPLETED);
    ...
    @Test
    void get_rejectsNonNaicFJob_with404() {
        ReportJob row = jobRow(UUID.randomUUID(), NaicScheduleFJobService.STATUS_COMPLETED);
    ...
    void reshapeFromJob_extractsPeriodFromParamsAndDelegatesShape() {
        ...
        when(shapingService.shape(ReportKey.NAIC_SCHEDULE_F,
                TENANT, PERIOD_START, PERIOD_END))
                .thenReturn(Mono.just(data));

        StepVerifier.create(service.reshapeFromJob(row))
                .assertNext(d -> assertThat(d.reportKey()).isEqualTo(ReportKey.NAIC_SCHEDULE_F))
                .verifyComplete();
    }
    ...
    private RegulatoryReportData emptyData() {
        return RegulatoryReportData.builder(
                        ReportKey.NAIC_SCHEDULE_F, TENANT, PERIOD_START, PERIOD_END, "USD")
                .build();
    }

    private RegulatoryReportData goldenData() {
        NaicScheduleFReportShaper shaper = new NaicScheduleFReportShaper(
                (t, ps, pe) -> Mono.just(new NaicScheduleFRawData(
