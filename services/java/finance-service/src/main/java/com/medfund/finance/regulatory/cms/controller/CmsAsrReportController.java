package com.medfund.finance.regulatory.cms.controller;
...
import com.medfund.finance.regulatory.cms.service.CmsAsrJobService;
...
/**
 * REST surface for the CMS Annual Statutory Return (Phase 16 §A, Phase 11).
 *
 * <ul>
 ...
 *   <li>{@link RequiresReport} — the CMS ASR toggle in tenant admin must
 *       be enabled (Rule from Phase 15).</li>
 *   <li>{@link RequiresPermission} — the calling user must hold either
 *       {@code finance:view} or {@code finance:export_regulatory}.</li>
 * </ol>
 */
@RestController
@RequestMapping("/api/v1/reports/regulatory/cms/asr")
@RequiredArgsConstructor
@Tag(name = "CMS Annual Statutory Return",
        description = "South Africa Council for Medical Schemes Annual Statutory Return "
                + "(Phase 16 §A). Async submit → poll → XLSX. Reporting currency is fixed at ZAR; "
                + "a non-blank override is rejected with 422 upstream by the shape service. "
                + "Bundled template is SYNTHETIC in Phase 11; a downstream sub-phase swaps in the "
                + "real template once obtained from CMS.")
@SecurityRequirement(name = "bearer-jwt")
public class CmsAsrReportController {

    private static final MediaType XLSX = MediaType.parseMediaType(
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");

    private final CmsAsrJobService jobService;
    ...
    @RequiresCountry({"ZA"})
    @RequiresReport(ReportKey.CMS_ASR)
    @RequiresPermission({Permissions.FINANCE_VIEW, Permissions.FINANCE_EXPORT_REGULATORY})
    @Operation(summary = "Submit a CMS Annual Statutory Return compute job",
            description = "Creates a report_job row for the requested year and kicks off the async "
                    ...
                    + "/api/v1/reports/regulatory/cms/asr/jobs/{jobId}/xlsx. "
    ...
    public Mono<ResponseEntity<CmsAsrJobSubmissionResponse>> submit(
            @Valid @RequestBody CmsAsrReportRequest body,
    ...
    @RequiresReport(ReportKey.CMS_ASR)
    @RequiresPermission({Permissions.FINANCE_VIEW, Permissions.FINANCE_EXPORT_REGULATORY})
    @Operation(summary = "Download the CMS Annual Statutory Return XLSX",
            description = "Re-shapes the report from the job's persisted params and composes the "
                    ...
                    + "belongs to another tenant or is not a CMS ASR job.")
    ...
    public Mono<ResponseEntity<byte[]>> exportXlsx(@PathVariable UUID jobId,
        ...
        return Mono.deferContextual(ctx -> {
            ...
            return jobService.get(jobId, tenantId)
                    .flatMap(job -> {
                        if (!CmsAsrJobService.STATUS_COMPLETED.equals(job.getStatus())
                                && !CmsAsrJobService.STATUS_FAILED.equals(job.getStatus())) {
                            return Mono.error(new ResponseStatusException(HttpStatus.CONFLICT,
                                    "Job is not in a terminal state yet: " + job.getStatus()));
                        }
                        if (CmsAsrJobService.STATUS_FAILED.equals(job.getStatus())) {
                            return Mono.error(new ResponseStatusException(HttpStatus.CONFLICT,
                        ...
                        return jobService.reshapeFromJob(job)
                                .flatMap(data -> xlsxService.render(tenantId, data)
                                        .flatMap(rendered -> emitExportSecurityEvent(tenantStr, jobId, jwt)
                                                .thenReturn(rendered)))
                                .map(rendered -> {
                                    String filename = "cms-asr-" + jobId + ".xlsx";
