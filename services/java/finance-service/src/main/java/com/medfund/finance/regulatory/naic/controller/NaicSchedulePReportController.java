...
import com.medfund.finance.regulatory.naic.service.NaicSchedulePJobService;
...
/**
 * REST surface for the NAIC Schedule P (Phase 16 §A, Phase 12).
 *
 * <ul>
 ...
 *   <li>{@link RequiresReport} — the NAIC Schedule P toggle in tenant
 ...
 *       isn't wired — Phase 12 default until Phase 14 lands).</li>
 * </ol>
 */
@RestController
@RequestMapping("/api/v1/reports/regulatory/naic/schedule-p")
@RequiredArgsConstructor
@Tag(name = "NAIC Schedule P",
        description = "United States NAIC Schedule P (Annual Statement, Phase 16 §A). Async submit "
                ...
                + "SYNTHETIC in Phase 12; a downstream sub-phase swaps in the real NAIC "
                + "Annual Statement Instructions template.")
@SecurityRequirement(name = "bearer-jwt")
public class NaicSchedulePReportController {

    private static final MediaType XLSX = MediaType.parseMediaType(
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");

    private final NaicSchedulePJobService jobService;
    ...
    @RequiresCountry({"US"})
    @RequiresReport(ReportKey.NAIC_SCHEDULE_P)
    @RequiresPermission({Permissions.FINANCE_VIEW, Permissions.FINANCE_EXPORT_REGULATORY})
    @Operation(summary = "Submit a NAIC Schedule P compute job",
            description = "Creates a report_job row for the requested year and kicks off the async "
                    ...
                    + "/api/v1/reports/regulatory/naic/schedule-p/jobs/{jobId}/xlsx. "
    ...
    public Mono<ResponseEntity<NaicSchedulePJobSubmissionResponse>> submit(
            @Valid @RequestBody NaicSchedulePReportRequest body,
    ...
    @RequiresReport(ReportKey.NAIC_SCHEDULE_P)
    @RequiresPermission({Permissions.FINANCE_VIEW, Permissions.FINANCE_EXPORT_REGULATORY})
    @Operation(summary = "Download the NAIC Schedule P XLSX",
            description = "Re-shapes the report from the job's persisted params and composes the "
                    ...
                    + "belongs to another tenant or is not a NAIC Schedule P job.")
    ...
    public Mono<ResponseEntity<byte[]>> exportXlsx(@PathVariable UUID jobId,
        ...
        return Mono.deferContextual(ctx -> {
            ...
            return jobService.get(jobId, tenantId)
                    .flatMap(job -> {
                        if (!NaicSchedulePJobService.STATUS_COMPLETED.equals(job.getStatus())
                                && !NaicSchedulePJobService.STATUS_FAILED.equals(job.getStatus())) {
                            return Mono.error(new ResponseStatusException(HttpStatus.CONFLICT,
                                    "Job is not in a terminal state yet: " + job.getStatus()));
                        }
                        if (NaicSchedulePJobService.STATUS_FAILED.equals(job.getStatus())) {
                            return Mono.error(new ResponseStatusException(HttpStatus.CONFLICT,
                        ...
                        return jobService.reshapeFromJob(job)
                                .flatMap(data -> xlsxService.render(tenantId, data)
                                        .flatMap(rendered -> emitExportSecurityEvent(tenantStr, jobId, jwt)
                                                .thenReturn(rendered)))
                                .map(rendered -> {
                                    String filename = "naic-schedule-p-" + jobId + ".xlsx";
