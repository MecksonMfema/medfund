...
/**
 * Composes the bundled (or tenant-override) PMB Spend template with the
 * shaped {@link RegulatoryReportData}, returning the XLSX bytes for the
 * HTTP download body + optional
 ...
 * {@link PmbCellMap} entries. If the resolved template comes from a
 * synthetic bundle ({@link TemplateSource#BUNDLED_SYNTHETIC}) the
 * {@link PmbSpendRenderResult#source} propagates so the Angular page
 * renders the synthetic-template warning banner.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class PmbSpendXlsxService {

    /** The regulator segment of the {@code report-templates/{regulator}/} classpath. */
    static final String REGULATOR = "pmb";
    /** The report-key segment inside that folder — {@code {reportKey}-v_YYYY-MM-DD.xlsx}. */
    static final String REPORT_KEY_FILE = "pmb-spend";

    private final RegulatoryTemplateService templateService;
    private final PmbCellMap cellMap;

    public Mono<PmbSpendRenderResult> render(UUID tenantId, RegulatoryReportData data) {
    ...
    private PmbSpendRenderResult renderToBytes(TemplateResolution resolution, RegulatoryReportData data) {
        Workbook wb = resolution.workbook();
        try {
            templateService.fill(wb, cellMap, PmbSpendReportShaper.toCellValueMap(data));
            byte[] bytes = templateService.toBytes(wb);
            return new PmbSpendRenderResult(bytes, resolution.source(), resolution.versionLabel());
        } finally {
            try {
                wb.close();
            } catch (Exception e) {
                log.warn("[pmb-xlsx] template workbook close failed: {}", e.getMessage());
            }
        }
    }

    /** Bytes + provenance so the controller can propagate the synthetic-banner hint. */
    public record PmbSpendRenderResult(
            byte[] bytes,
