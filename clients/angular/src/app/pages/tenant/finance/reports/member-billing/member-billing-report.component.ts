 ...
 * TRAVEL, DISABILITY, VEHICLE, PROPERTY, individual HEALTH). The
 * symmetry-fix for scheme + group billing surfaces per Phase 3 §8.
 */
@Component({
  selector: 'app-member-billing-report',
  standalone: true,
  imports: [CommonModule, FormsModule, IconComponent, SelectComponent, DataTableComponent],
  templateUrl: './member-billing-report.component.html',
...
export class MemberBillingReportComponent implements OnInit {
  ...
  readonly columns: TableColumn[] = [
    { key: 'memberNumber',      label: 'Member #',    sortable: false },
    ...
    { key: 'contributionCount', label: 'Rows',        sortable: false },
  ...
  exportExcel(): void {
    ...
    this.exporting = true;
    this.finance.exportMemberBillingExcel(this.buildParams()).subscribe({
      next: blob => {
        downloadBlob(blob, `billing-members-${this.periodStart}-to-${this.periodEnd}.xlsx`);
  ...
  onRowClick(row: MemberBillingSummaryRow): void {
    this.router.navigate(['/tenant/finance/reports/member-billing', row.memberId],
