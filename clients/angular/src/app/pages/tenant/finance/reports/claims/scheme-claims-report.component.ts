...
/**
 * Per-scheme claims aggregate — one row per (scheme, currency) with the
 * claimed / approved / paid funnel (G42). Period clock is adjudicated_at
 * (G41); rows stay native-currency (G25). Clicking a row drills into the
 * scheme's monthly buckets + claim ledger.
 */
@Component({
  selector: 'app-scheme-claims-report',
  standalone: true,
  imports: [CommonModule, FormsModule, IconComponent, SelectComponent, DataTableComponent, ReportBackButtonComponent],
  templateUrl: './scheme-claims-report.component.html',
  styleUrl: './claims-report.component.scss',
})
export class SchemeClaimsReportComponent implements OnInit {
  loading = false;
  exporting = false;
  ...
  fetch(): void {
    ...
    this.loading = true;
    this.claimsReport.getClaimsPerScheme(this.buildParams()).subscribe({
      next: env => {
        this.envelope = env;
        this.rows = env.data ?? [];
        this.loading = false;
  ...
  exportExcel(): void {
    ...
    this.exporting = true;
    this.claimsReport.exportClaimsPerSchemeExcel(this.buildParams()).subscribe({
      next: blob => {
        downloadBlob(blob, `claims-schemes-${this.periodStart}-to-${this.periodEnd}.xlsx`);
  ...
  private surfaceWarnings(env: ReportResponse<ClaimsSummaryRow[]>): void {
    ...
    this.toast.warning(composeWarningsToast(warnings, 'Claims report: per scheme'), 8000);
  }

  onRowClick(row: ClaimsSummaryRow): void {
    this.router.navigate(['/tenant/finance/reports/claims-scheme', row.dimensionId],
