 ...
 * executed payment run (V080, D6). Periodless: rows are native
...
export class MemberBalanceHistoryComponent implements OnInit {
  loading = false;
  exporting = false;
  ...
  fetch(): void {
    this.loading = true;
    this.finance.getMemberBalanceHistory(this.memberId, this.buildParams()).subscribe({
      next: env => {
        this.envelope = env;
        this.loading = false;
  ...
  exportExcel(): void {
    this.exporting = true;
    this.finance.exportMemberBalanceHistoryExcel(this.memberId, this.buildParams()).subscribe({
      next: blob => {
        downloadBlob(blob, `member-balance-history-${this.memberId}.xlsx`);
  ...
  private surfaceWarnings(env: ReportResponse<BalanceHistoryResponse>): void {
    ...
    this.toast.warning(composeWarningsToast(warnings, 'Member balance history'), 8000);
  }
