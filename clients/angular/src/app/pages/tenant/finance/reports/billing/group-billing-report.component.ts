...
export class GroupBillingReportComponent implements OnInit {
  ...
  fetch(): void {
    ...
    this.finance.getGroupBillingReport(this.buildParams()).subscribe({
      ...
      error: err => {
        this.toast.error(extractErrorMessage(err, 'Failed to load group billing report'));
        this.rows = [];
        this.envelope = null;
        this.loading = false;
      },
  ...
  private surfaceWarnings(env: ReportResponse<GroupBillingSummaryRow[]>): void {
    const warnings = env?.warnings ?? [];
    if (warnings.length === 0) return;
    this.toast.warning(composeWarningsToast(warnings, 'Billing report: per holder'), 8000);
  }
