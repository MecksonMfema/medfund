...
/**
 * Per-scheme receipts aggregate — one row per (scheme, currency). Rows stay
 * native-currency per G25. The synthetic "Unallocated group payments" scheme
 * row (dimensionId=null) captures group-owned transactions without a
 * contribution back-link (G33); clicking it opens the unallocated drill-down.
 */
@Component({
...
export class SchemeReceiptsReportComponent implements OnInit {
  loading = false;
  exporting = false;
  ...
  readonly columns: TableColumn[] = [
    { key: 'dimensionName',   label: 'Scheme',        sortable: false },
  ...
  fetch(): void {
    ...
    this.loading = true;
    this.finance.getSchemeReceiptsReport(this.buildParams()).subscribe({
      next: env => {
        this.envelope = env;
        this.rows = env.data ?? [];
        this.loading = false;
  ...
  exportExcel(): void {
    ...
    this.exporting = true;
    this.finance.exportSchemeReceiptsExcel(this.buildParams()).subscribe({
      next: blob => {
        downloadBlob(blob, `receipts-schemes-${this.periodStart}-to-${this.periodEnd}.xlsx`);
  ...
  private surfaceWarnings(env: ReportResponse<ReceiptsSummaryRow[]>): void {
    ...
    this.toast.warning(composeWarningsToast(warnings, 'Receipts report: per scheme'), 8000);
  }

  onRowClick(row: ReceiptsSummaryRow): void {
    // Unallocated bucket → route to the drill-down with an unallocated flag.
    if (!row.dimensionId) {
      this.router.navigate(['/tenant/finance/reports/receipts-scheme/unallocated'],
        { queryParams: this.periodParams() });
      return;
    }
    this.router.navigate(['/tenant/finance/reports/receipts-scheme', row.dimensionId],
      { queryParams: this.periodParams() });
  }
