...
export class PropertiesListComponent implements OnInit {
  ...
  readonly columns: TableColumn[] = [
    ...
    { key: 'sumInsured',      label: 'Sum insured', sortable: true, type: 'currency' },
  ...
  readonly actions: TableAction[] = [
    {
      ...
      icon: 'eye',
      color: 'default',
      handler: (row: PropertyRow) =>
        this.router.navigate(['/tenant/policies/properties', row.id]),
    },
  ];
  ...
  fetchPage(): void {
    this.loading = true;
    this.policies.listPropertiesPaged({
      ...
      next: (resp: PoliciesPageResponse<PropertyRow>) => {
      ...
      error: (err) => {
        this.toast.error(extractErrorMessage(err, 'Failed to load properties'));
        this.rows = [];
        this.totalCount = 0;
        this.totalPages = 1;
        this.loading = false;
